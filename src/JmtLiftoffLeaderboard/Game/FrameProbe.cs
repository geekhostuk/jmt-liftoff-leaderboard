...
/// was this plugin's own work.
///
/// A stall of a few tens of milliseconds is what a pilot feels as the drone clipping
/// something it should have missed, and it leaves no other trace. So every entry point the
/// game calls this plugin through is timed, and a frame well over the usual is written to
/// the BepInEx log with this plugin's share of it, and whether a garbage collection ran.
/// Every five minutes of flying a summary follows, so a quiet log reads as "measured and
/// fine" rather than "not measured".
///
...
internal static class FrameProbe
{
    ...
    private static readonly string[] Names = { "HUD and menus", "room updates", "gate checks", "site replies" };
    ...
    /// <summary>Times one call into this plugin. Nested calls count once, as the outermost part.</summary>
    ...
    /// which is while the pilot is on a course.
    /// </summary>
    ...
    private static void Count(long now, double frameMs, bool collected)
    {
        ...
        var line = $"Perf: slow frame: {frameMs:0} ms, where frames usually take {usual:0.0} ms. " +
                   $"This plugin's code ran {oursMs:0.00} ms of it{Breakdown(Spent)}." +
    ...
    private static void Summarise(long now)
    {
        if (_frames > 0)
        {
            ...
            Plugin.Log.LogInfo(
                $"Perf: last {minutes:0} min on a course: {_frames} frames, usually {_usualMs:0.0} ms; " +
                $"{_slow} slow" + (_slow > 0 ? $" (worst {_worstMs:0} ms, {_slowWithCollection} with a garbage collection)" : "") +
                $". This plugin's code: {_oursMs / _frames:0.000} ms a frame on average, " +
    ...
    /// <summary>" (room updates 0.21 ms, HUD and menus 0.05 ms)", biggest first, or nothing when it rounds to nothing.</summary>
